...
namespace Fortizan\Tekton\DependencyInjection\Compiler\Projection;
...
class ProjectionHandlerPass implements CompilerPassInterface
{
    ...
    {
        if (!$container->hasDefinition('tekton.bus.event.locator')) {
            return;
        }

        $targetBus = 'tekton.bus.event';

        $locatorDefinition = $container->getDefinition('tekton.bus.event.locator');

        $handlers = $container->findTaggedServiceIds('tekton.projection.handler');
